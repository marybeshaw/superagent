...
Request.prototype._redirect = function(res){
  ...
  let changesOrigin = parse(url).host !== parse(this.url).host;
  if (changesOrigin) {
    let domainRegex = /(?:[a-zA-Z0-9][a-zA-Z0-9-]*\.)*([a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9]\.[a-zA-Z]{2,})(?::\d+)?/g;
    let newUrlMatch = domainRegex.exec(parse(url).host);
    let oldUrlMatch = domainRegex.exec(parse(this.url).host);

    // allow sub domains that share a domain
    if (newUrlMatch && newUrlMatch[1] && oldUrlMatch && oldUrlMatch[1]) {
      changesOrigin = newUrlMatch[1] !== oldUrlMatch[1];
    }
    // allow localhost:5000
    else if (parse(this.url).host === 'localhost:5000') {
      changesOrigin = false;
    }
  }
