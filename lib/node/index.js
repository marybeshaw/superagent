...
Request.prototype._redirect = function(res){
  ...
  if (shouldStripCookie) {
    var domainRegex = /(?:[a-zA-Z0-9][a-zA-Z0-9-]*\.)*([a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9]\.[a-zA-Z]{2,}|localhost)(?::\d+)?/g;
    var newUrlMatch = domainRegex.exec(parse(url).host);

    // reset the regex
    domainRegex.lastIndex = 0;

    var oldUrlMatch = domainRegex.exec(parse(this.url).host);

    // allow sub domains that share a domain
    if (newUrlMatch && newUrlMatch[1] && oldUrlMatch && oldUrlMatch[1]) {
      shouldStripCookie = Boolean((newUrlMatch[1] !== oldUrlMatch[1]) && (oldUrlMatch[1] !== 'localhost'));
    }
  } 
